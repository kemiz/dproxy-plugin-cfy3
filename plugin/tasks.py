...
def wait(**kwargs):
    ...
    if ('wait_for' not in ctx.node.properties or
    ...
    if ctx.node.properties['wait_for'] == 'exists':
        while time.time() <= endtime:
            try:
                ...
                val = client.deployments.outputs.get(ctx.node.properties['deployment_id']).outputs[
                    ctx.node.properties['test']]

                if val is not None:
                    ctx.instance.runtime_properties['outputs'] = client.deployments.outputs.get(
                        ctx.node.properties['deployment_id']).outputs
                    return
            except:
    ...
    # handle "expr"
        ...
        while time.time() <= endtime:
            try:
                outputs = client.deployments.outputs.get(
                    ctx.node.properties['deployment_id']).outputs

                ctx.logger.info("evaluating {0}".format(ctx.node.properties['test']))
                if eval(ctx.node.properties['test']):
